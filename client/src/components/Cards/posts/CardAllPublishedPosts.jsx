...
const CardAllPublishedPosts = ({ color, activeDisplay, search, pageLimit, leftArrow, rightArrow }) => {

        
    // ****************************************************************************
    // MANAGE STATE:-  TO FIND ALL PUBLISHED POSTS
    // ****************************************************************************
    const [publishedBlogPosts, setPublishedBlogPosts] = useState([]);
    console.log("ALL PUBLISHED BLOG POSTS: ", publishedBlogPosts); 

    // eslint-disable-next-line
    const [totalPublishedBlogPosts, setTotalPublishedBlogPosts] = useState(null);
    console.log("TOTAL PUBLISHED BLOG POSTS: ", totalPublishedBlogPosts);
    ...
    useEffect(() => {
        const allPublishedPostsLink = document.querySelector("#postsLinkID .publishedPosts");
        // console.log("ALL PUBLISHED POSTS LINK", allPublishedPostsLink);
        if (activeDisplay === "publishedPosts") {
            allPublishedPostsLink?.classList.add("activePostView");
            ...
            async function fetchAllPublishedPosts() {
                ...
                await api.get(`/api/v1/admin/posts/manage?page=${currentPage}&limit=${pageLimit}&status=${publishedPosts}`)
                .then((response) => {
                    ...
                    setPublishedBlogPosts(allBlogPosts);
                   
                    setTotalPublishedBlogPosts(pagination?.postsRecord);
            ...
            var timerID = setTimeout(fetchAllPublishedPosts, 300);   // Delay execution of findAllStaffs by 1800ms
            return () => {
                clearTimeout(timerID);                  // Clean up timer if component unmounts or token changes
            };
        } else {
            allPublishedPostsLink?.classList.remove("activePostView");
        };
    ...
    if (isLoading) {        
        return (        
            <div className={`w-full overflow-x-auto ${activeDisplay === "publishedPosts" ? "block" : "hidden"}`}>
        ...
        <>
            ...
            <div className={`w-full overflow-x-auto ${activeDisplay === "publishedPosts" ? "block" : "hidden"}`}>
                                        ...
                                        <table className="items-center w-full bg-transparent border-collapse">
                                            ...
                                            {search(publishedBlogPosts)?.length !== 0 ?
                                                    <tbody>                                                    
                                                        {search(publishedBlogPosts)?.map((post, userIndex) => {
...
export default CardAllPublishedPosts;
