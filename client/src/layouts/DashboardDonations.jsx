...
const DashboardDonations = ({ color, isLoggedIn }) => {
    ...
    // console.log("TOTAL DONATIONS: ", totalDonations);
        const [totalPublishedDonations, setTotalPublishedDonations] = useState(null);
        // console.log("TOTAL PUBLISHED DONATIONS: ", totalPublishedDonations);
        const [totalDraftDonations, setTotalDraftDonations] = useState(null);
        // console.log("TOTAL DRAFT DONATIONS: ", totalDraftDonations);
    ...
    async function fetchAllDonations() {     
        ...
        await api.get(`/api/v1/admin/donations/manage/publishedDonations`)
        .then((response) => {
            const { success, data, message } = response.data;
            if (!success && message === "No donation found") {
                console.log("Success: ", success);
            ...
            setTotalPublishedDonations(data);
        })
        .catch((error) => {
            console.log("Error fetching data: ", error);
        });
        ...
        await api.get(`/api/v1/admin/donations/manage/draftDonations`)
        .then((response) => {
            const { success, data, message } = response.data;              
            if (!success && message === "No donation found") {
                console.log("Success: ", success);
                            ...
                            <div className="w-full mb-12 px-4">
                                <div
                                    ...
                                    <div id="donationsLinkID" className="flex flex-row flex-wrap gap-3 mt-8 mb-10 px-7">
                                        ...
                                        <Link className="publishedDonations pt-3 pb-2 px-10 rounded-lg border mr-2 text-xl flex flex-row gap-1 bg-white" onClick={() => setActiveDisplay("publishedDonations")}>Published  <span className="off_white"> ({ totalPublishedDonations })</span></Link>
                                        <Link className="draftDonations pt-3 pb-2 px-10 rounded-lg border mr-2 text-xl flex flex-row gap-1 bg-white" onClick={() => setActiveDisplay("draftDonations")}>Drafts  <span className="off_white"> ({ totalDraftDonations })</span></Link>
                                        {/* <Link className="scheduledDonations pt-3 pb-2 px-10 rounded-lg border text-xl flex flex-row gap-1 bg-white" onClick={() => setActiveDisplay("scheduledPosts")}>Scheduled  <span className="off_white"> ({ totalDonations })</span></Link> */}
                                    </div> 
                                    {/* Donations Navigation */}
