...
exports.monitorUsersActivities = async (req, res) => {
    ...
    try {
        ...
        const log_user_actitivities = await recordActivity.save();
        console.log("USER ACTIVITY: ", log_user_actitivities.log);

        const responseData = {       
            servermessage: log_user_actitivities.log,
        };        
...
exports.verifySignUpWithGetReq = async (req, res) => {
    ...
    if (verified_data) {
        ...
        res.redirect(`https://${ip}/user/signup?token=${verifiedUser.acesstoken}&email=${verifiedUser.email}&verified=${verified_success}&message=${encodeURIComponent(verified_message)}`);
    } else {
